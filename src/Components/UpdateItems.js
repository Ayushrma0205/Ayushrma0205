...
function UpdateItems() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:5000/displayitems/${id}`)
            .then(response => {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            veg__non_veg: displayItem?.veg__non_veg || '',
            size: displayItem?.size || '',
            ingredients: displayItem?.ingredients || '',
            cheese: displayItem?.cheese || '',
        ...
        validationSchema: Yup.object({
            ...
            size: Yup.string().required('Size (Small, Medium, Large, Extra Large) is required'),
            ingredients: Yup.string().required('Please enter some ingredients'),
            cheese: Yup.string().required('Cheese (Yes or No) is required'),
            description: Yup.string()
        }),

        onSubmit: (values, { setSubmitting, resetForm, setStatus }) => {
            axios.put(`http://localhost:5000/displayitems/${id}`, values)
                .then(response => {
                    setStatus('success');
                    resetForm();
    ...
    if (!displayItem) return <div>Loading...</div>;
